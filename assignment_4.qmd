...
Vi velger å benytte tabellen med kode nama_10r_3gdp, med følgende forklarende tekst: «Gross domestic product (GDP) at current market prices by NUTS 3 regions».
...
eu_data_nested |> 
  ...
  # ag_comment: lagt til make_date for å få fin horisontal akse
...
eu_data_nested |> 
  ...
  unnest(NUTS2_data) |> 
  select(NUTS2, year, gini_nuts2, gdp_per_capita) |> 
  flextable() |> 
  width(1, width = 1.5) |> 
  width(2, width = 1.5)
```
